...
int main(int argc, char *argv[]) {
    ...
    if(argv[1][0] == '-') {
        switch(argv[1][1]) {
        case 'b': if (number_nonblank(argv[2], line)) return 0; break;
        case 'E': if (e_gnu_flag(argv[2], line)) return 0; break; 
        case 'e': if (e_posix_flag(argv[2], line)) return 0; break;  
        case 'n': if (number(argv[2], line)) return 0; break;
        // case 's':
        // case 't':
        } 
    } else { if (none_flags(argv[1], line)) return 0; }
}
